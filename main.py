...
CYPHER_TO_GREMLIN = os.getenv("CYPHER_TO_GREMLIN")
...
def load_query_lookup():
    global QUERY_LOOKUP
    jsonl_path = Path(__file__).parent / CYPHER_TO_GREMLIN
    if not jsonl_path.exists():
        logger.warning("{} not found.".format(CYPHER_TO_GREMLIN))
        return
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            QUERY_LOOKUP[normalize(obj["query"])] = obj["gremlin_query"]

    print(f"Loaded {len(QUERY_LOOKUP)} query mappings")


def add_query_lookup(cypher_query: str, gremlin_query: str):
    QUERY_LOOKUP[normalize(cypher_query)] = gremlin_query
...
def execute_query(id: str, body: QueryRequest, _=Depends(require_auth)):
    ...
    print("\n========== execute_query() ==========")
    print("Graph ID:", id)
    print("Query Language:", service.query_language)
    print("Body Query:", body.query)
    print("Body Prompt:", body.prompt)
    print("=====================================\n")

    logger.warning("execute_query() called")
    logger.warning("Query language: %s", service.query_language)
    logger.warning("Body query: %s", body.query)
    logger.warning("Body prompt: %s", body.prompt)
    # ---------------------------------------
    ...
    if body.prompt:
        ...
        logger.warning("Generated query:\n%s", generated["query"])
    ...
    # Gremlin backend
    #
    if body.query and service.query_language == "gremlin":
        print(">>> Entered GREMLIN branch")
        print("Normalized query:", normalize(body.query))
        for q_key in QUERY_LOOKUP.keys():
            print("KEY:", q_key)
        cached_query = QUERY_LOOKUP.get(normalize(body.query))
        if cached_query:
            print(">>> CACHE HIT")
            logger.warning("Gremlin query fetched from lookup.")
            translated = {
                "query": cached_query,
            ...
            print(">>> CACHE MISS")
            logger.warning("Gremlin query not found. Calling LLM.")
            schema_context = service.get_schema_context(id)
            ...
            logger.warning("Gremlin query added to lookup.")

        print("\n========== EXECUTING GREMLIN ==========")
        print(translated["query"])
        print("Parameters:", translated["parameters"])
        print("=======================================\n")

        logger.warning("Executing Gremlin:\n%s", translated["query"])
    ...
    #
    # PostgreSQL / AGE translation
    #
    if body.query and service.query_language != "cypher":

        print(">>> Entered TRANSLATION branch")
        print("Target:", service.query_language)
        ...
        print("\n========== EXECUTING TRANSLATED QUERY ==========")
        print(translated["query"])
        print("Parameters:", translated["parameters"])
        print("================================================\n")

        logger.warning("Translated query:\n%s", translated["query"])
    ...
    # Native Cypher
    #
    print(">>> Entered NATIVE CYPHER branch")
...
def execute_write(id: str, body: WriteRequest, _=Depends(require_auth)):
    ...
    #
    # Gremlin backend -> Lookup first
    #
    if service.query_language == "gremlin":
        cached_query = QUERY_LOOKUP.get(normalize(body.query))
        if cached_query:
            logger.info("Gremlin query fetched from lookup.")
            ...
            logger.info("Gremlin query not found in lookup. Calling LLM...")
        ...
        logger.info(
            "Cypher -> Gremlin write: %s | params=%s",
            translated["query"],
            translated["parameters"],
        )
