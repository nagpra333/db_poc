...
#
# `by` is deliberately NOT included here. Unlike has/property, two-argument
# by('key', <direction>) is the ORDER BY form, where the second argument is
# a Gremlin Order enum token (incr/decr/asc/desc), never a bound variable --
# see _BY_ORDER_VAR_REF_RE below, which checks it separately.
_TWO_ARG_VAR_REF_RE = re.compile(
    r"\.(?:has|hasNot|property|where|is|constant|option)\s*\(\s*"
    r"'[^']*'\s*,\s*([A-Za-z_][A-Za-z0-9_]*)\s*\)"
)

# Two-argument by('key', X) -- the sort-direction form used after order().
# X is almost always incr/decr/asc/desc (an Order enum token), but a real
# bound variable is technically possible, so only filter out known
# direction tokens rather than assuming every match here is safe.
_BY_ORDER_VAR_REF_RE = re.compile(
    r"\.by\s*\(\s*'[^']*'\s*,\s*([A-Za-z_][A-Za-z0-9_]*)\s*\)"
)
_ORDER_DIRECTION_TOKENS = frozenset({"incr", "decr", "asc", "desc"})
...
def _extract_referenced_vars(query_text: str) -> set[str]:
    ...
    by_order = {
        m for m in _BY_ORDER_VAR_REF_RE.findall(query_text)
        if m not in _ORDER_DIRECTION_TOKENS
    }
    one_arg = {
        m for m in _ONE_ARG_VAR_REF_RE.findall(query_text)
        if m not in _GREMLIN_NON_VAR_TOKENS
    }
    return two_arg | by_order | one_arg
