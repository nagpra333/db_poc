...
Neo4j implementation of GraphAdapter (see graph_adapter.py). Selected when
DB_TYPE=neo4j. FastAPI route handlers (main.py) never talk to the driver
directly -- they call methods on Neo4jGraphService through the adapter
interface.
...
# Local aliases so the rest of this file reads the same as before the refactor.
_sanitize_ident = sanitize_ident
_now_iso = now_iso
...
class Neo4jGraphService(GraphAdapter):
