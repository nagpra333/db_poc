...
def validate_environment() -> None:
    ...
    if db_type not in required:
        raise EnvironmentError(
            f"Unsupported DB_TYPE '{db_type}'. Expected 'neo4j' or 'gremlin' or 'apache_age'."
        )

    missing = [var for var in required[db_type] if not os.getenv(var)]

    # Azure OpenAI is required because llm.py depends on it
    azure_required = [
...
def _build_service() -> GraphAdapter:
    ...
    if DB_TYPE == "neo4j":
        ...
        return ApacheAgeGraphService(
            ...
            database=os.getenv("AGE_DATABASE"),
            user=os.getenv("AGE_USERNAME"),
            password=os.getenv("AGE_PASSWORD"),
        )
    else:
        raise RuntimeError(f"Unsupported DB_TYPE={DB_TYPE!r}; expected 'neo4j' or 'gremlin' or 'apache_age'.")
