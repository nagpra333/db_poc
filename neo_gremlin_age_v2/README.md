# Graph Database API v2 -- FastAPI + Neo4j / Cosmos DB Gremlin / Apache AGE
...
Backed by **Neo4j**, **Azure Cosmos DB's Gremlin API**, or **Apache AGE**,
...
-   `llm.py` -- Azure OpenAI client + query generation/validation,
    ...
    into that backend's dialect. Neo4j backends skip the LLM entirely
    for `query` bodies.
...
## Option C: Apache AGE (`DB_TYPE=apache_age`)
